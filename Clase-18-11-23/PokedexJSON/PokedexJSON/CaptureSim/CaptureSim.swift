...
//    manda a llamar cuando carga la escena
    override func didMove(to view: SKView) {
        ...
        gameTimer = Timer.scheduledTimer(timeInterval: 0.8,
                                         target: self,
    ...
    private func placeAnimal() {
        ...
        guard let animalName = animals.randomElement(), let animalImage = UIImage(systemName: animalName) else {return}
...
extension CaptureScene: SKPhysicsContactDelegate {
    func didBegin(_ contact: SKPhysicsContact) {
        if contact.bodyA.node?.name == "pokeball" || contact.bodyB.node?.name == "pokeball" && contact.bodyB.node?.name == "sun.max.circle" || contact.bodyA.node?.name == "sun.max.circle" {
//            captured pokemon
